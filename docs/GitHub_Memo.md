...
リモートリポジトリ作成 >> クローンしてローカルリポジトリ作成 >> ブランチ作成 >> ファイル修正 >>addで変更をステージに上げる >> コミット >> originをmasterにプル >> プッシュ >>プルリクエストを出す >> マージする >> リモートリポジトリのブランチを削除 >> ローカルリポジトリでプルする >> ローカルリポジトリのブランチを削除

## リモートリポジトリの作成

GitHubのWebページでリポジトリを作成する

リポジトリのページの右側にある`Clone or download`をクリックし，URL横のボタンをクリックしてリポジトリのURLをコピーする
...
フォルダ内のファイルを見ると **.git**フォルダが作られているのが確認できる
...
`-global`オプションをつけると他のリポジトリでもデフォルトでユーザがセットされるので毎回ユーザ情報をセットする必要がなくなる
...
ファイルが変更されているか、削除されているかなどが確認できる

ワークツリー⇔ステージ間の変更と，リポジトリ⇔ステージ間の変更の両方が示される
...
基本的にmasterでは作業しないので新しいブランチを作成して，そこで作業を行う 
...
## commitとstaging、変更も取り消す。(前回のコミット状態に戻す)  
